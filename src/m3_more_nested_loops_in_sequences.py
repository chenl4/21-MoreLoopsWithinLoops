...
def run_test_largest_number():
    ...
    # Test 4:
    expected = 43
    answer = largest_number([[1, 4], [2, 5, 43, 13]])
    print('Expected and actual are:', expected, answer)
...
def largest_number(seq_seq):
    ...
    for k in range(len(seq_seq)):
        for j in range(len(seq_seq[k])):
            if type(seq_seq[k][j]) == float or int:
                numbers = numbers + [seq_seq[k][j]]
    if len(numbers) == 1:
        return numbers[0]
    elif len(numbers) < 1:
        return None
        ...
        for i in range(1, len(numbers)):
            if numbers[i] > numbers[k_for_max]:
                k_for_max = i
...
def run_test_largest_negative_number():
    ...
    # Test 3:
    expected = -2
    answer = largest_negative_number([[1, 4, -2, 6], [2], [4, 2, -5]])
    print('Expected and actual are:', expected, answer)

    # Test 4:
    expected = -24
    answer = largest_negative_number([[124, 532, -235], (124, -24, 432)])
    print('Expected and actual are:', expected, answer)
...
def first_is_elsewhere_too(seq_seq):
    ...
    for k in range(len(seq_seq[0])):
        init = init + [seq_seq[0][k]]

    for j in range(len(init)):
        for i in range(1, len(seq_seq)):
            for o in range(len(seq_seq[i])):
                if init[j] == seq_seq[i][o]:
                    return True
